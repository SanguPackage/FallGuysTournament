...
export function sanguMark(size: number): string {
...
export function levelLink(map: string): string {
  ...
  return `<a class="sangu" href="${SANGU}levels/${name}" target="_blank" rel="noopener" title="Clips and shortcuts on sangu.be">${sanguMark(26)}</a>`;
}
