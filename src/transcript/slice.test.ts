...
test("a show's lines run from its start, inclusive, to the next show's, exclusive", () => {
  expect(linesBetween(EVENING, at("21:00:00"), at("21:05:00")).map((entry) => entry.at)).toEqual([
    at("21:00:00"),
    at("21:02:00"),
  ]);
});
...
test("the show still being played takes everything from its start on", () => {
  expect(linesBetween(EVENING, at("21:00:00"), Infinity).map((entry) => entry.at)).toEqual([
    at("21:00:00"),
    at("21:02:00"),
    at("21:05:00"),
  ]);
...
test("a line with no clock belongs to the server, not to a show", () => {
  expect(linesBetween(EVENING, 0, Infinity).map((entry) => entry.at)).toEqual([
    at("20:59:00"),
    at("21:00:00"),
    at("21:02:00"),
    at("21:05:00"),
